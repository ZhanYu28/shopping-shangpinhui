    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <DetailZoom :skuImageList="skuImageList" />
          <!-- 小图列表 -->
          <DetailImageList :skuImageList="skuImageList" />
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="spuSaleAttr in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd
                  changepirce="0"
                  v-for="(spuValue,index) in spuSaleAttr.spuSaleAttrValueList"
                  :class="{active:spuValue.isChecked == 1}"
                  :key="spuValue.id"
                  @click="changeChecked(index,spuSaleAttr.spuSaleAttrValueList)"
                >
...
<script setup>
// 组件
import DetailImageList from "./ImageList/ImageList";
import DetailZoom from "./Zoom/Zoom";
...
const { getGoodsInfo } = useStoreActions("detail", ["getGoodsInfo"]);
const { goodsInfo } = useStoreState("detail", ["goodsInfo"]);
const { categoryView, skuInfo, spuSaleAttrList } = useStoreGetters("detail", [
  "categoryView",
  "skuInfo",
  "spuSaleAttrList",
]);

const skuImageList = computed(() => skuInfo.value.skuImageList);

// 路由实例
const route = useRoute();

// 属性点击事件
function changeChecked(index,arr){
  arr.forEach(item => {
    item.isChecked = "0"
  })
...
function changeSkuNum(event){
  let inputNum = event.target.value * 1;
  if(isNaN(inputNum) || inputNum <1){
    skuNum.value = 1
  }else{
    skuNum.value = parseInt(inputNum)
  }
...
async function addShopCart(){
  try {
    await addOrUpdataShopCart({skuId:route.params.skuid,skuNum:skuNum.value})
    sessionStorage.setItem('SHOPINFO',JSON.stringify(skuInfo.value))
...
onMounted(() => {
  getGoodsInfo(route.params.skuid);
});
